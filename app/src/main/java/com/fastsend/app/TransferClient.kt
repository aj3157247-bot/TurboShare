...
class TransferClient(
    ...
    fun send(host: String, files: List<SelectedFile>) {
        pool.execute {
            try {
                Socket().use { socket ->
                    socket.connect(InetSocketAddress(host, TransferProtocol.PORT), 5000)
                    ...
                    files.forEach { f ->
                        ...
                        resolver.openInputStream(f.uri).use { TransferProtocol.writeString(out, TransferProtocol.sha256(it!!)) }
                    }
                    out.flush()
                    ...
                    for (f in files) {
                        ...
                        resolver.openInputStream(f.uri).use { stream ->
                            ...
                            while (done < f.size) {
                                ...
                                if (done % (4L * TransferProtocol.BUFFER) == 0L) out.flush()
                            }
                        }
                        out.flush()
                        history.add(TransferItem(f.name, f.size, "SENT", System.currentTimeMillis(), true))
                    }
                    onState("Transfer complete")
                }
            } catch (e: Exception) {
                onState("Transfer failed: ${e.message ?: "connection error"}")
            }
        }
    }
